...
PV_IMAGE_ENABLED="${5:-'True'}"  # 'True' if image previews are enabled, 'False' otherwise.
...
handle_image() {
    ...
    case "${mimetype}" in
        ...
        # Image
        image/*)
            exists catimg && catimg -r 2 -c -w 148  $FILE_PATH
            #exists catimg && catimg -c -t -r 1 -w 100 $FILE_PATH
            local orientation
...
handle_mime() {
    ...
    case "${mimetype}" in
        ...
        image/*)
            ...
            [ -z which catimg ] && catimg "${FILE_PATH}" && exit 5
            [ -z which viu ] && viu -t "${FILE_PATH}" && exit 5
            exiftool "${FILE_PATH}" && exit 5
            exit 1;;
